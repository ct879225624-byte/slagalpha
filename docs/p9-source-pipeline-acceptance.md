...
加入历史来源一致性检查后，两项显式验收全部通过，耗时 342.98 秒
（本机实测，不是生产性能保证；此前正式入口接线验收为 475.55 秒）：
...
- 本次逐位来源 JSON 共 3,110,476 bytes；包含本次规范化凭证，不是跨运行的尺寸不变量，
  该小样本尺寸不能外推为全 DEV 容量保证。
- 默认回归最近一次为 788 passed、1 skipped；Ruff、mypy（含 acceptance）146 文件、
