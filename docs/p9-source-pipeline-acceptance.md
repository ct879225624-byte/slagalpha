...
前一轮整日入口验收实际使用 CPython 3.12.14，因此不代表冻结环境通过。本轮跨日入口
验收已在隔离 CPython 3.12.13 和 29 个冻结 wheel 下运行，环境锁通过；未修改锁或迁移
现有 `.venv`。隔离解释器来自 Astral `python-build-standalone`，因为 Python.org 不为
3.12.13 提供 Windows 二进制安装包。
...
- 按日期和扫描位惰性生成四周期前缀，调用正式
  `compute_source_bound_request_set_with_checkpoints`；该入口跨日共享来源链、逐日调用
  `compute_source_bound_scan_day` 并保存，内部通过 `compute_source_bound_scan_slot`
  计算 P3–P6。验收代码只提供输入流，完整请求集合由生产入口直接返回。
...
改用正式跨日计算入口后，两项显式验收全部通过，耗时 495.86 秒
（本机实测，不是生产性能保证；此前整日入口验收为 568.69 秒）：
...
- 默认回归最近一次为 836 passed、1 skipped；Ruff、mypy（含 acceptance）148 文件、
