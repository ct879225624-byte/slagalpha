...
现固定为 `parameters/0.1.0:<sha256>`；内容工件和计划归属校验见
`docs/p9-parameter-version-contract.md`。正式执行器仍必须调用该校验，不能只接受格式正确
的字符串。纯构造函数也不会确认调用者填写的提交真的存在，实际 Git 观测由前检负责。
...
参数版本工件及独立计划绑定校验已实现，但当前有限前检没有选择具体候选；正式执行器
集成、完整输入内容核验、实际事件时刻的历史规则、1m/Funding、依赖工件哈希等仍需
后续验证，不能拿该状态启动锁定测试。
